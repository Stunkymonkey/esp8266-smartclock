...
void createServer() {
  ...
  server.on("/settings", []() {
    ...
    for(int i=0; i<(sizeof configSocketSets / sizeof configSocketSets[0]); i++) {
      ...
      content += "<form action='/socketSet' method='GET'><h4>Socket ";
      content +=  String(i+1);
      content += "</h4>";
      ...
      content += "<label for='name'>Name of socket</label><input id='name' type='text' name='name' placeholder='Name for socket' value='"+socketSet[0]+"'>";
      content += "<label for='houseCode'Housecode</label><input type='text' id='houseCode' name='houseCode' placeholder='Housecode' value='"+socketSet[1]+"'>";
      content += "<label for='groupCode'Housecode</label><input type='number' id='groupCode' name='groupCode' placeholder='Gruppe (optional)' value='"+socketSet[2]+"'>";
      content += "<label for='socketCode'Socketcode</label><input type='number' id='socketCode' name='socketCode' placeholder='Device Code' value='"+socketSet[3]+"'>";
      content += "<input type='submit' value='Speichern'>";
      content += "</form><br>";
    }
    sendResponse(content);
