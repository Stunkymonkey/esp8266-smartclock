...
void sendResponse(String content) {
  ...
  if (ENABLE_VICTRON) {
    finalContent += "<a href=\"/victron\" class=\"icon solarIcon\"></a>";
  }
...
void Victron() {
  content = "";
  content += "<container>";
  content += "<h1>Victron Solar Interface</h1>";
  content += "<h3>Battery</h3>";
  content += "<div class=\"victronContainer\">";
  content += addVictronItem(V, "Battery Voltage", "mV");
  content += addVictronItem(I, "Battery Current", "mA");
  content += "</div>";

  content += "<h3>Panel</h3>";
  content += "<div class=\"victronContainer\">";
  content += addVictronItem(VPV, "Panel Voltage", "mV");
  content += addVictronItem(PPV, "Panel Power", "W");
  content += "</div>";

  content += "<h3>Load</h3>";
  content += "<div class=\"victronContainer\">";
  content += addVictronItem(LOAD, "Load State", "");
  content += addVictronItem(IL, "Load Current", "mA");
  content += "</div>";

  content += "<h3>History</h3>";
  content += "<div class=\"victronContainer\">";
  content += addVictronItem(H20, "Yield today in 0.01kWh", "");
  content += addVictronItem(H21, "Maximum Power today", "W");
  content += addVictronItem(H22, "Yield yesterday in 0.01kWh", "");
  content += addVictronItem(H23, "Maximum Power yesterday", "W");
  content += "</div>";

  content += "<pre>";
  content += "Firmware Version: "+String(value[FW])+"\n";
  content += "Day sequence number: "+String(value[HSDS])+"\n";
  content += "Yield total: "+String(value[H19])+"\n";
  content += "</pre>";

  content += "</container>";
  sendResponse(content);
}
String addVictronItem(int id, String description, String unit) {
  if(id < 0) {
    print("Wrong identifier for Victron value");
    return "";
  }
  String newItem = "<div class=\"victronItem\">";
  newItem += String(value[id]);
  newItem += unit;
  newItem += "<span>";
  newItem += description;
  newItem += "</span></div>";
